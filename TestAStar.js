...
function aStarSearch(graph, start, goal, heuristics, timeout) {
    var pQueue = new collections.PriorityQueue(function (Object1, Object2) {
    ...
    //Initial queue edges
    var startEdges = graph.outgoingEdges(start);
    ...
    while (!pQueue.isEmpty()) {
        ...
        var s = pQueue.dequeue();
        if (goal(s.path[s.path.length - 1])) {
            return s;
        }
        //Add neighbouring edges to queue
        var adjEdges = graph.outgoingEdges(s.path[s.path.length - 1]);
        for (var i = 0; i < adjEdges.length; i++) {
            ...
            var lowerInClosed = false;
            var storedFValue = closedSet.getValue(adjEdges[i].from);
        ...
        closedSet.setValue(s.path[s.path.length - 1], s.cost + heuristics(s.path[s.path.length - 1]));
    }
    return result;
}
function newTemp(s1) {
